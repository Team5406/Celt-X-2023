...
import frc.team5406.robot.lib.swervelib.math.SwerveMath;
...
import frc.team5406.robot.lib.swervelib.simulation.SwerveIMUSimulation;
...
public class SwerveDrive
{
  ...
  private boolean initPass = false;
      ...
      SwerveDriveConfiguration config, SwerveControllerConfiguration controllerConfig)
  {
    ...
    for(SwerveModule module : swerveModules){
      if(module.moduleError()){
        System.out.println("Module Error: " + module.moduleError());
        error++;
      }
    }
    if(error == 0){
    ...
    SmartDashboard.putBoolean("Swerve Subsystem Init", initPass);
  }
  ...
  public double[] getDriveModulePosition(){
    return new double[] {
      swerveModules[0].getDrivePosition(),
      swerveModules[1].getDrivePosition(),
      swerveModules[2].getDrivePosition(),
      swerveModules[3].getDrivePosition()
  };
}
}
